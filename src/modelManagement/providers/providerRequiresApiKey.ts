 ...
 * every creation path (BYOK setup, agent setup) and backfilled onto legacy rows
 * by the settings-v5 migration. Self-hosting is NOT the criteria — a
 * self-hosted endpoint can sit behind an auth proxy, and a hosted provider can
 * be keyless — so the answer is always the stored flag.
 *
...
export function providerNeedsResolvedApiKey(provider: Provider): boolean {
  return providerRequiresApiKey(provider) || !!provider.apiKeyKeychainId;
}
