...
describe("ChatButtons lifecycle-action gating", () => {
  it.each([USER_SENDER, "ai"])("opens available sources for a %s message", (sender) => {
    ...
    fireEvent.click(screen.getByTitle("Show Sources"));
    expect(onShowSources).toHaveBeenCalledTimes(1);
    rerender(
      <TooltipProvider>
        <ChatButtons message={message(sender)} hasSources={false} onShowSources={onShowSources} />
      </TooltipProvider>
    );
    expect(screen.queryByTitle("Show Sources")).toBeNull();
  });
