...
// Type-only barrel import (runtime-erased): the shared row/group shapes live in
// the agent-mode UI layer, which host code reaches through the barrel.
...
export interface CandidatePartition {
  /** BYOK configured models (opencode candidates only). */
  byokCandidates: Candidate[];
 ...
 * models matching `agentType`, plus BYOK models for opencode only.
 *
 * For opencode, a BYOK provider it can't route (`isOpencodeRoutable` false —
 * self-hosted) is dropped to avoid a dead toggle: `opencodeEnabledModelEntries`
 * skips unroutable providers, so enabling one would never reach the agent or
 * picker.
 */
export function partitionCandidates(
  ...
  const byokCandidates: Candidate[] = [];
  ...
  for (const configuredModel of configuredModels) {
    ...
    // BYOK rows are candidates for opencode only — and only when opencode can
    // actually route the provider (no dead toggles).
    if (isOpencode) {
      if (isOpencodeRoutable && !isOpencodeRoutable(provider)) continue;
      byokCandidates.push(candidate);
    }
  }
  return { byokCandidates, agentOriginCandidates };
}

/**
 * Partition for the non-agent "chat" backend (Quick Chat). Every BYOK
 * configured chat model is a candidate, bucketed one-group-per-provider by
 * `buildModelEnableGroups`. Agent-origin models are excluded — the chat backend
 * instantiates via LangChain (`ChatModelManager`), which can't drive an agent
 * CLI's models — and embedding models are excluded since they aren't chat
 * models.
 */
 ...
 * persist (claude, codex); BYOK and opencode carry none and so render a
...
function originBadgeLabel(kind: OriginKind): string {
  if (kind === "agent") return "Agent Provided";
  return "BYOK";
}
...
/**
 * Provider-grouped rows for the shared list. BYOK candidates get one group per
 ...
 * origin (opencode mixes BYOK/agent) we tag every group with an origin badge to
 * disambiguate; a single-origin list (claude/codex) gets none.
 */
export function buildModelEnableGroups(
  ...
  // BYOK providers — one group per provider.
  const byProvider = new Map<string, { label: string; kind: OriginKind; rows: ModelEnableRow[] }>();
  for (const candidate of partition.byokCandidates) {
  ...
  // Every origin gets a badge only when the list mixes origins.
