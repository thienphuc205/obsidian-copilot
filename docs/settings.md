...
These are optional web search and page-fetch tools for **Codex Agent sessions only**. They are separate from **Self-Host → Web Search Provider** and its **Firecrawl API Key**. They do not require a Copilot Plus license; you need a signed-in Codex account for Agent Chat and credentials for **Firecrawl**, **Tavily**, **Exa**, or your own **Custom API**. The selected service may charge its own credits.

To set them up, turn on **Enable Codex Agent web tools**, select a provider, enter that provider's key, and select **Test Connection**. The test runs only after that explicit click; it sends a small search request to the selected service and may consume credits. The key field is masked and uses your configured API key storage. Keys are saved separately: switching providers preserves the other providers' keys.

Every option supports separate search and page-fetch tools with clickable web sources in the current Agent turn. Tavily uses basic Search and Extract requests; Exa uses Search and Contents. **Custom API** requires a public HTTPS base URL that implements Copilot's normalized `/search` and `/fetch` format; it is not an OpenAI-compatible chat endpoint. See [Custom Web API](custom-web-api.md) for the request and response contract. A provider error does not switch to another service automatically. The source links shown with tool results are currently available in the live session; saved chat history does not retain that tool-result source list.

Changing the enablement, provider, API key, or Custom API base URL restarts the Codex Agent backend when it is running and can interrupt an active turn. To roll back, turn the setting off. New Codex turns will not receive these web tools. Turning the setting off does not delete saved keys. To remove the selected provider's key, select **Remove key** next to the key field; you can do this while web tools are off, and it does not enable them. Use **Advanced → API Key Storage → Delete All Keys** only when you intend to remove every Copilot credential for this vault.

The selected provider receives the queries and URLs used by these tools, and an agent-generated query can contain vault data. This setting is not a vault sandbox and does not guarantee that requests are vault-free. Codex's configured remote model service may also receive the prompt context and tool results needed for a turn. The opt-in **Strict @file/@folder context** setting filters explicit Quick Chat selections at the current-vault read boundary; it does not enforce an OS sandbox or constrain Agent ACP shell, filesystem, MCP, or remote-model capabilities.

This preview has not been live-validated against Firecrawl, Tavily, Exa, or a Custom API. A local test or gallery success state does not prove that the selected service is reachable from your device. Existing Copilot-hosted, BYOK, Self-Host, opencode, and Claude routes are unchanged by this opt-in setting.
...
| **Strict @file/@folder context** | Off              | Opt-in lexical boundary for explicit Quick Chat file/folder context. Current-vault file references are re-resolved, stale or unsafe paths are omitted, and folder matching uses exact path boundaries. This does not sandbox ACP shell/filesystem/MCP tools or remote-model inference. |
...
| **Document Processor**           | **Plus**                                   | Controls document routes rather than one shared parser. In Agent Chat, an explicitly installed experimental `pdfjs/` package is probed first on desktop for local, page-aware text extraction; a complete package bypasses the legacy PDF cache and Miyo/Plus, while a missing package leaves the normal route unchanged. **Miyo** runs the local `miyo-parse` CLI for PDF and EPUB and fails closed if that CLI is unavailable; a remote Miyo server is not used. Quick Chat instead asks the connected Miyo service to parse those formats, so a configured remote server processes them remotely. **Plus** can use Copilot-hosted PDF processing and paid usage. Outside the Miyo route, EPUB and ordinary non-PDF formats such as DOCX are unsupported in regular chat context; Projects use a separate conversion route. The selector remains available while Miyo is disconnected. See [the experimental local PDF package](pdf-manual-install.md) for its manual delivery and privacy limits. |
